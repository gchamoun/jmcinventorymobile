...
class InventoryService {
  ...
  Future<Item> getItem(int itemId) async {
    ...
    Response response =
        await http.post(baseUrl + 'mobile/getItem/' + itemId.toString());
    print(response.data);
  ...
  Future<bool> checkoutItem(int itemId, bool firstItem) async {
    ...
    Response response = await http.post(baseUrl +
        'mobile/checkOutItems/' +
  ...
  Future<List<Item>> getUsersItems(int userId) async {
    ...
    final response = await http
        .post(baseUrl + 'mobile/getUsersCheckin/' + userId.toString());
    if (response.data != '') {
  ...
  Future<bool> checkinItem(int workerCheckinId, int itemId) async {
    ...
    Response response = await http.post(baseUrl +
        'mobile/individualCheckin/' +
        workerCheckinId.toString() +
        '/' +
        itemId.toString());
  ...
  Future<bool> getReservation(int itemId) async {
    ...
    Response response = await http
        .post(baseUrl + 'inventory/mobile_checkin/' + itemId.toString());
    print(response.data);
    return (response.data == 1) ? true : false;
  }
}
